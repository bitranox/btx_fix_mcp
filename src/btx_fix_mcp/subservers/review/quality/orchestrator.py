...
class AnalyzerOrchestrator:
    ...
    def initialize_analyzers(self) -> None:
        ...
        self.test_analyzer = TestSuiteAnalyzer(
            self.repo_path, self.logger, analyzer_config
        )
    ...
    def build_analyzer_tasks(
        ...
        if features.test_analysis:
            tasks.append(
                ("tests", self.test_analyzer.analyze, python_files, ["tests"])
            )

        # Architecture analyzer
        if (
            features.architecture_analysis
            or features.runtime_check_detection
            or features.import_cycle_detection
        ):
            tasks.append(
                (
                    "architecture",
        ...
        # Metrics analyzer
        if (
            features.halstead_metrics
            or features.raw_metrics
            or features.code_churn
        ):
            tasks.append(
                (
                    "metrics",
        ...
        # Type analyzer
        if (
            features.type_coverage
            or features.dead_code_detection
            or features.docstring_coverage
        ):
            tasks.append(
                (
                    "types",
    ...
    def execute_tasks(
        self, tasks: list[tuple[str, Any, list[str], list[str]]]
    ) -> dict[str, Any]:
        ...
        def run_analyzer(
            task: tuple[str, Any, list[str], list[str]]
